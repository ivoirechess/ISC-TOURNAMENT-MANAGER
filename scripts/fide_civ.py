...
def sql_document(players, report, batch_size):
    ...
    lines.extend([
        ...
        "update public.players p set official_fide_data=to_jsonb(s) from fide_civ_stage s where p.fide_id=s.fide_id;",
        "insert into public.players(" + columns + ") select " + columns + " from fide_civ_stage on conflict(fide_id) do update set name=case when players.local_overrides?'name' then players.name else excluded.name end,federation=case when players.local_overrides?'federation' then players.federation else excluded.federation end,fide_title=case when players.local_overrides?'fide_title' then players.fide_title else excluded.fide_title end,other_titles=case when players.local_overrides?'other_titles' then players.other_titles else excluded.other_titles end,fide_active=case when players.local_overrides?'fide_active' then players.fide_active else excluded.fide_active end,rating_std=case when players.local_overrides?'rating_std' then players.rating_std else excluded.rating_std end,rating_rapid=case when players.local_overrides?'rating_rapid' then players.rating_rapid else excluded.rating_rapid end,rating_blitz=case when players.local_overrides?'rating_blitz' then players.rating_blitz else excluded.rating_blitz end,source_period=excluded.source_period,fide_synced_at=excluded.fide_synced_at,normalized_name=case when players.local_overrides?'name' then players.normalized_name else excluded.normalized_name end,source=excluded.source;",
...
def main(argv=None):
    ...
    report = {
        ...
        "potential_duplicates": len(reconciliation["potential_duplicates"]), "protected_override_fields": sum(len(row.get("local_overrides", {})) for row in existing if row.get("fide_id") in reconciliation["exact_fide_ids"]), "duplicate_fide_ids": duplicate_ids,
